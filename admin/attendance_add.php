<?php
	...
	if(isset($_POST['add'])){
		...
		if($query->num_rows < 1){
			...
			if($query->num_rows > 0){
				$_SESSION['error'] = 'Registro de asistencia existe actualmente';
			}
			else{
				//updates
				...
				if($conn->query($sql)){
					$_SESSION['success'] = 'Registro de asistencia guardado con éxito';
					$id = $conn->insert_id;
		...
		$_SESSION['error'] = 'Primero llena el formulario de adición';
	}
